$(document).ready(function () {
            ...
            success: function (res) {
                ...
                if (questions.length) {
                    ...
                    questions.forEach((q, i) => {
                        ...
                        const earnedPoints = Number(q.earnedPoints ?? 0);
                        const maxPoints = Number(q.maxPoint ?? 0);
                        const resultCell = isSelfConfidence
                            ? `<span class="fw-bolder" style="font-size:1rem; color:#14532d;">${escapeHtml(String(earnedPoints))}/${escapeHtml(String(maxPoints))}</span>`
                            : (q.isCorrect
    ...
    function drawRadarChart(categories, chartData) {
        ...
        const option = {
            ...
            radar: {
                indicator: reversedCategories.map((name, index) => ({
                    name: name,
                    max: 100,
                    textStyle: {
                    fontFamily: '"Nunito Sans", sans-serif',
