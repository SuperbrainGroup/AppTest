...
(function (window) {
    ...
    function buildResultTableParts(categories, chartData) {
        ...
        let headRow = "<tr><th class=\"text-center\">STT</th><th>Khóa học</th><th class=\"text-center\">Ngày kiểm tra</th>";
        cats.forEach((c) => {
        ...
        chartData.forEach((item, index) => {
            body += `<tr><td class="text-center">${index + 1}</td><td>${escapeHtml(item.name)}</td><td class="text-center">${escapeHtml(item.date)}</td>`;
    ...
    const PRINT_STYLES_A4 = `
                ...
                .print-wireframe.result-print-a4 { border: none !important; border-radius: 0; padding: 2mm 4mm !important; }
                ...
                .wf-quote-print { margin-top: 4px; font-size: 9px; line-height: 1.4; padding: 6px 8px; max-height: 76mm; overflow: hidden; }
                .exam-print-layout { display: flex; flex-direction: column; min-height: 272mm; box-sizing: border-box; }
                .exam-print-top { flex-shrink: 0; }
                .exam-print-title { text-align: center; font-size: 11px; font-weight: 800; color: #183153; margin-bottom: 6px; letter-spacing: 0.3px; }
                .exam-student-row-3 { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 6px; font-size: 9px; margin-bottom: 6px; }
                .exam-student-row-3 > div { border: 1px solid #e2e8f0; padding: 5px 7px; border-radius: 4px; background: #f8fafc; }
                .exam-info-lbl { display: block; font-size: 8px; font-weight: 700; letter-spacing: 0.5px; color: #64748b; text-transform: uppercase; margin-bottom: 2px; }
                .exam-q-compact-wrap { display: flex; flex-direction: column; gap: 5px; flex: 1 1 auto; min-height: 0; }
                .exam-q-compact { border: 1px solid #e5e7eb; padding: 4px 8px; border-radius: 4px; background: #fcfdff; }
                .exam-q-line { display: flex; flex-direction: row; align-items: baseline; justify-content: space-between; gap: 10px; }
                .exam-q-main { flex: 1 1 auto; min-width: 0; font-size: 9px; line-height: 1.35; color: #253247; }
                .exam-q-prefix { font-weight: 800; color: #183153; }
                .exam-q-compact .exam-q-text { font-weight: 400; color: #253247; }
                .exam-q-compact .exam-q-pts { flex: 0 0 auto; font-size: 8px; color: #64748b; white-space: nowrap; }
                /* In đề thi: ảnh nên chiếm full width để tiết kiệm giấy và nhìn rõ hơn */
                .exam-q-compact .exam-q-img { width: 100% !important; max-width: 100%; height: auto; max-height: 160px; object-fit: contain; display: block; margin-top: 4px; }
                .exam-print-a4 .exam-section-title { font-size: 9px; margin: 4px 0 4px 0; }
                .exam-print-a4 .exam-footer-note { margin-top: auto; flex-shrink: 0; padding-top: 8px; margin-bottom: 0; border-top: 1px solid #e2e8f0; font-size: 7.5px; color: #94a3b8; text-align: center; }
                @media print {
                    html, body { margin: 0; }
                    .exam-print-layout { min-height: 275mm; }
                }
            `;
